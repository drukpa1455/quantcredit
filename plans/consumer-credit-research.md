...
### Can one artifact serve learning, research, and interview rehearsal?
...
- **Decision:** The toolkit is executable study material, not an assumed
  assessment dependency. Every helper has a small explicit contract and may be
  explained or rewritten directly if an interview environment disallows
  personal packages.
...
- A hidden "solve the assessment" function or reliance on `quantcredit` being
  installable during an interview. The project teaches transparent operations;
  it does not bypass assessment rules.
- Using an agent, external reference, or personal code when an assessment does
  not permit it. The notebook is always preparation material and becomes a live
  aid only under the interviewer's stated rules.
...
statistical vocabulary. XGBoost remains a useful interview comparison but is
...
- **D-13:** Practice with pandas, scikit-learn's classical gradient boosting,
  ...
  during an assessment. XGBoost practice is separate because its resolved ARM
...
bounded; a timed assessment can pass narrower tuples without changing semantics.
...
- **O-3 — Primary decision statistic:** Recommend validation log loss with
  calibration and average precision as co-gates; report AUROC for interview and
